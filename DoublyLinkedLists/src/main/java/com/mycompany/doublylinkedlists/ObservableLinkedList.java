...
public class ObservableLinkedList<V> implements Iterable<V> {
    ...
    private Observer<Container> observer;

    public void setObserver(Observer<Container> observer) {
    ...
    public ObservableLinkedList(V[] template) {
    ...
    public void replaceValueAtIndex(V value, int index) {
        ...
        Container oldValue = new Container();
        oldValue.value = current.value;
        current.value = value;

        if (observer != null) {
            observer.modified(oldValue, current);
        }
