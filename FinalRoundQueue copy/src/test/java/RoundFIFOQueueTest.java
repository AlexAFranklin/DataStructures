...
public class RoundFIFOQueueTest {
    ...
    public void addingInts() {
        ...
        intFifo.dequeue();
        intFifo.enqueue(800, false);
        assertThrows(RuntimeException.class, () -> {
            intFifo.enqueue(10101, false);
        });
    ...
    public void addingString() {
        ...
        assertEquals(5, strFifo.size());
        assertThrows(RuntimeException.class, () -> {
            strFifo.enqueue("Angel", false);
        });
        strFifo.dequeue();
        strFifo.enqueue("Angel", false);
        assertThrows(RuntimeException.class, () -> {
            strFifo.enqueue("Riley", false);
        });
    ...
    public void arrayOfInts() {
        ...
        intFifo.enqueue(303, true);
        intFifo.enqueue(202, true);

        int[] expected3 = {202, 303, 10, 349, 186};
        Integer[] lifoArray3 = intFifo.toArray();
        iterator = 0;
        for (Integer num : lifoArray3) {
            assertEquals(expected3[iterator++], num);
        }
        iterator = 0;
        for (Integer num : intFifo) {
            assertEquals(expected3[iterator++], num);
    ...
    public void arrayOfStrings() {
        ...
        strFifo.enqueue("Cordelia", true);
        strFifo.enqueue("Riley", true);

        String[] expected3 = {
            "Riley", "Cordelia", "Anya", "Tara", "Willow"
        };
        String[] lifoArray3 = strFifo.toArray();

        iterator = 0;
        for (String name : lifoArray3) {
            assertEquals(expected3[iterator++], name);
        }
        iterator = 0;
        for (String name : strFifo) {
            assertEquals(expected3[iterator++], name);
